...
const impactoColor = { 'Alto': 'text-emerald-400', 'Médio-alto': 'text-cyan-400', 'Médio': 'text-amber-400' }
const viabilidadeColor = { 'Alta': 'text-emerald-400', 'Média': 'text-amber-400', 'Baixa': 'text-rose-400' }
            ...
            >
              {propostasPriorizadas.map(({ icon: Icon, color, titulo, descricao, impacto, viabilidade, referencia }, i) => (
                <motion.div
                  ...
                  className="glass rounded-2xl p-5 border border-white/8 flex gap-4 group hover:border-white/15 transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-start justify-between gap-2 mb-1">
                      <h4 className="font-bold text-slate-100">{titulo}</h4>
                      <div className="flex gap-2 flex-shrink-0">
                        <span className="text-xs px-2 py-0.5 rounded-md bg-white/5 border border-white/8">
                    ...
                    <p className="text-sm text-slate-400 leading-relaxed mb-1">{descricao}</p>
              ...
              {sucessos.map(({ nome, resultado, destaque }, i) => (
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-100 mb-0.5">{nome}</h4>
                      <span className="inline-block text-xs font-semibold bg-emerald-500/15 text-emerald-300 px-2 py-0.5 rounded-md">{destaque}</span>
                    </div>
                  </div>
                  <p className="text-sm text-slate-400 leading-relaxed pl-8">{resultado}</p>
                </motion.div>
              ))}
            </motion.div>
          )}
            ...
            >
              <div className="space-y-3 mb-6">
                {fracassos.map(({ nome, razao }, i) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-200">{nome}</p>
                      <p className="text-sm text-slate-400">{razao}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
              <div className="glass-amber rounded-2xl p-5 border border-amber-500/20">
                <p className="text-sm text-amber-200 leading-relaxed">
                  <span className="font-bold text-amber-300">Padrão identificado:</span> Todos os projetos falharam por falta de continuidade, foco apenas educativo sem ação estrutural, ou por não atacar a causa raiz. Qualquer proposta nova precisa considerar isso explicitamente no seu design.
                </p>
              </div>
            </motion.div>
          )}

...
const colorMap = {
  cyan:   { glass: 'glass-cyan',    icon: 'text-cyan-600 dark:text-cyan-400',   border: 'border-cyan-500/20'   },
  amber:  { glass: 'glass-amber',   icon: 'text-amber-600 dark:text-amber-400',  border: 'border-amber-500/20'  },
  rose:   { glass: 'glass-rose',    icon: 'text-rose-600 dark:text-rose-400',   border: 'border-rose-500/20'   },
  violet: { glass: 'glass',         icon: 'text-violet-600 dark:text-violet-400', border: 'border-violet-500/20' },
}
    ...
    <section id="problema" className="py-24 px-4 sm:px-6 lg:px-8 relative">
      <div className="max-w-7xl mx-auto" ref={ref}>
          ...
          {cards.map(({ icon: Icon, color, title, content, items }, i) => {
            ...
            return (
              <motion.div
                ...
                className={`${c.glass} rounded-2xl p-6 flex flex-col gap-3 border ${c.border} hover:border-opacity-50 hover:bg-slate-900/[0.04] dark:hover:bg-white/[0.06] transition-colors`}
              >
                <div className={`w-10 h-10 rounded-xl bg-slate-900/5 dark:bg-white/5 flex items-center justify-center ${c.icon}`}>
                  <Icon className="w-5 h-5" />
                </div>
                <h3 className="font-bold text-slate-900 dark:text-slate-100">{title}</h3>
                {content && <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">{content}</p>}
                {items && (
        ...
        <motion.div
          ...
          className="glass rounded-2xl p-6 flex items-start gap-4 border border-slate-200 dark:border-white/8"
        >
          ...
          <div>
            <p className="font-semibold text-slate-800 dark:text-slate-200 mb-1">Embasamento acadêmico publicado</p>
            <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
              Esta situação é documentada em pesquisa publicada especificamente sobre este bairro:{' '}
              <span className="text-violet-700 dark:text-violet-300 font-medium">Magalhães; Cseh; Gonçalves-Dias (2022)</span> —
              conflitos na coleta de resíduos no Jardim Keralux — e{' '}
              <span className="text-violet-700 dark:text-violet-300 font-medium">Magalhães; Gonçalves-Dias (2023)</span> —
